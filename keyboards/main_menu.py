...
SECTION_HELP = {
    ...
    "photo": (
        "<b>Анализ фотографии (EXIF)</b>\n\n"
        "Извлекает скрытые метаданные из фото:\n"
        "- GPS координаты → точное место съёмки\n"
        "- Модель камеры / смартфона\n"
        "- Дата и время съёмки\n"
        "- Настройки камеры (ISO, выдержка)\n"
        "- Оценка риска приватности\n\n"
        "⚠️ <b>Важно: отправляй файлом, не фото!</b>\n"
        "Telegram сжимает обычные фото и удаляет все метаданные.\n\n"
        "📎 <b>Как отправить файлом:</b>\n"
        "• Телефон: Скрепка → <b>Файл</b> → выбери фото\n"
        "• ПК: перетащи фото с зажатым <b>Shift</b>\n"
        "  или выбери «Отправить без сжатия»\n\n"
        "🔲 <b>Проверка QR-кода:</b>\n"
        "Тоже отправь <b>файлом</b> (без сжатия)\n"
        "или используй команду <code>/qr</code>"
    ),
}
